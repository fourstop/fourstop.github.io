...
(function ($) {
	...
	function isSideMenuPage() {
		if ($body.hasClass('p-asidemenupage')) return isAsideMenuPage = true;
	}
	isSideMenuPage();
	...
	$(window).smartresize(function () {
		...
		if ($('.portfolio-masonry, .s-blog-masonry').length) {
			//refresh masonry layout, then do our work
			$('.portfolio-masonry, .s-blog-masonry').one('layoutComplete', function () {
				_onresize();
			});
		}
		/* else {
	...
	var scrolling = {
		...
		headerObserve: function () {
			if (_scroll >= this.offsetToChangeHeader && !this.headerModified) {
		...
		updateOffsets: function () {
			...
			this.offsetToChangeHeader = this.section[0].top === 0 ? this.section[0].height - this.headerHeight : 0;
		},
	...
	function menuSticky() {
		if (isSmallScreen() && !isAsideMenuPage) $('.header').addClass('header-sticky header-dark');
	}
	menuSticky();
